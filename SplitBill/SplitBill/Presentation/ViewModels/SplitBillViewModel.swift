...
class SplitBillViewModel: ObservableObject {
    ...
    init(billTitle: String = "", totalAmount: String = "", scannedItems: [(name: String, price: Double)] = [], scannedAdjustments: [(name: String, amount: Double)] = []) {
        ...
        print("DEBUG ViewModel: Received \(scannedAdjustments.count) adjustments")

        // Add scanned adjustments
        for adjustment in scannedAdjustments {
            print("DEBUG ViewModel: Adding adjustment: \(adjustment.name) - \(adjustment.amount)")
            self.adjustments.append(Adjustment(name: adjustment.name, amount: adjustment.amount, isDiscount: false))
        }

        print("DEBUG ViewModel: Total adjustments in array: \(self.adjustments.count)")
    }
    ...
    func addPerson(name: String) {
        ...
        // BUT: Don't auto-split if there are scanned items waiting to be manually assigned
        let uniqueAmounts = Set(people.map { $0.amount })
        let canEvenSplit = uniqueAmounts.count <= 1 && totalAmountDouble > 0 && scannedItems.isEmpty
        if canEvenSplit {
    ...
    func removeItem(id: UUID) {
        ...
        if let item = items.first(where: { $0.id == id }) {
            // Restore to scanned items if not already there
            restoreToScannedItems(name: item.name, price: item.price)
        }
    ...
    func removeItem(name: String, price: Double, personId: UUID) {
        if let idx = items.firstIndex(where: { $0.personId == personId && $0.name == name && $0.price == price }) {
            // Restore to scanned items
            restoreToScannedItems(name: name, price: price)
            items.remove(at: idx)
            recalcTotals()
        }
    }

    func restoreToScannedItems(name: String, price: Double) {
        // Check if item already exists in scanned items
        let alreadyExists = scannedItems.contains(where: {
            $0.name.lowercased() == name.lowercased() && abs($0.price - price) < 0.01
        })

        // Only restore if it doesn't already exist
        if !alreadyExists {
            scannedItems.append((name: name, price: price))
            print("DEBUG: Restored '\(name)' to scanned items")
        }
    }
